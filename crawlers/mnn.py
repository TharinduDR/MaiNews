#!/usr/bin/env python3
"""
News scraper for https://maithilinewsnetwork.com/  (a WordPress site).
...
-------------------------------------------
This site PERMITS automated collection:
  * robots.txt (HestiaCP default) has no Disallow rules, no AI opt-out, and no
    content signals -- it only sets `Crawl-delay: 10`.
  * the WordPress REST API returns 200 to normal requests.
  * the server is plain nginx (no Cloudflare bot challenge).
So this scraper needs no circumvention. It DOES honour the site's stated
crawl delay (10s between requests by default, auto-read from robots.txt) and
sends a truthful, identifiable User-Agent.

Strategies
----------
  --method api    WordPress REST API /wp-json/wp/v2/posts  (default, clean)
  --method html   sitemap discovery + parse `.article-content`  (fallback)

Usage
-----
    python mnn_scraper.py                       # API, all posts
    python mnn_scraper.py --limit 20            # quick test
    python mnn_scraper.py --method html
    python mnn_scraper.py --out articles --delay 10
    python mnn_scraper.py --contact you@uni.edu # add your email to User-Agent
...
BASE = "https://maithilinewsnetwork.com"
API_POSTS = BASE + "/wp-json/wp/v2/posts"
DEFAULT_DELAY = 10.0          # matches the site's robots.txt Crawl-delay

CONTENT_SELECTORS = [".article-content", ".entry-content", "article"]
JUNK_SELECTORS = [
    ".mag-box", ".mini-posts-box", ".related-posts", ".post-bottom-meta",
    ".sharedaddy", ".addtoany_share_save_container", ".wp-block-buttons",
    ".sharedaddy", ".jp-relatedposts", "script", "style", "ins", "iframe",
    ".code-block", ".heateor_sss_sharing_container", "figure.wp-block-embed",
]
NON_ARTICLE = (
...
def build_user_agent(contact: str | None) -> str:
    base = "MNN-research-collector/1.0 (academic; low-resource Maithili NLP"
...
def read_crawl_delay(session) -> float | None:
    """Respect the crawl-delay the site advertises in robots.txt."""
    rp = RobotFileParser()
    rp.set_url(BASE + "/robots.txt")
    try:
        rp.read()
        cd = rp.crawl_delay(session.headers.get("User-Agent", "*")) \
            or rp.crawl_delay("*")
...
def is_article_url(url: str) -> bool:
    # Permalinks here are /<category>/<slug>/ (multi-segment), so we don't
    # filter on segment count -- only exclude obvious non-article paths.
...
def parse_article_html(html_text: str, url: str) -> dict:
    ...
    if not author:
        a = soup.select_one("a[rel=author], .author-name, .meta-author, .td-post-author-name a")
...
# --------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Scrape maithilinewsnetwork.com to JSON.")
    ap.add_argument("--method", choices=["api", "html"], default="api")
    ap.add_argument("--out", default="articles")
    ap.add_argument("--limit", type=int, default=None)
    ap.add_argument("--delay", type=float, default=None,
                    help="Seconds between requests (default: site's robots crawl-delay, else 10).")
    ...
    # Honour the site's advertised crawl-delay unless the user overrides it.
    delay = args.delay
    if delay is None:
    ...
    if args.method == "api":
        try:
            ...
            print(f"\nREST API failed ({e}). Falling back to HTML...\n", file=sys.stderr)
