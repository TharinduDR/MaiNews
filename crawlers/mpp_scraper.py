#!/usr/bin/env python3
"""
News scraper for https://mppdainik.com/  (a WordPress site).
...
-------------------------------------------
  * robots.txt (standard WordPress/WooCommerce/Yoast) disallows only admin and
    WooCommerce internal paths; articles are allowed. No AI opt-out, no content
    signal. It advertises a sitemap at /sitemap_index.xml.
  * the REST API returns 200 at the standard /wp-json/ endpoint.
  * the server is plain LiteSpeed (no Cloudflare bot challenge).
So this is ordinary, permitted access -- no circumvention. No crawl-delay is
advertised, so the scraper defaults to a courteous 2s between requests and
...
BASE = "https://mppdainik.com"
DEFAULT_DELAY = 2.0           # no robots crawl-delay advertised; be courteous
...
CONTENT_SELECTORS = [".cm-entry-summary", ".cm-post-content", ".entry-content", ".post-content", "article"]
JUNK_SELECTORS = [
...
def build_user_agent(contact):
    base = "MPP-research-collector/1.0 (academic; low-resource Maithili NLP"
...
# --------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Scrape mppdainik.com to JSON.")
    ap.add_argument("--method", choices=["api", "html"], default="api")
    ap.add_argument("--out", default="articles")
    ap.add_argument("--limit", type=int, default=None)
    ap.add_argument("--delay", type=float, default=None,
                    help="Seconds between requests (default: robots crawl-delay, else 2).")
